...
export class LLMOptimizer {
  ...
  private async withRetries<T>(
    ...
    while (attempt < maxRetries) {
      try {
        ...
        p.log.warn(
          pc.yellow(
            `[LLM] API call failed. Attempt ${attempt}/${maxRetries}. Retrying...`,
          ),
        );
  ...
  async enhanceReadme(idea: string, currentReadme: string): Promise<string> {
    ...
    if (!this.model) {
      p.log.warn(
        pc.gray(`[LLM] API key not found. Skipping README refinement.`),
      );
...
Return ONLY the raw markdown content. No conversational text.
      ...
      spinner.stop(
        pc.yellow(
          `LLM enhancement failed: ${err instanceof Error ? err.message : String(err)}. Falling back to default.`,
        ),
      );
      return currentReadme;
    }
