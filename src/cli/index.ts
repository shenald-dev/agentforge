...
program
  ...
  .option(
    "--no-llm",
    "Skip LLM-based README enhancement even if API key is set",
  )
  .action(async (idea, options) => {
    ...
    try {
      ...
      const templatePath = await templateManager.getTemplatePath(
        answers.template,
      );
      ...
      if (options.llm !== false) {
        ...
        try {
          ...
          const enhancedReadme = await optimizer.enhanceReadme(
            answers.idea,
            currentReadme,
          );
      ...
      await new Promise<void>((resolve) => {
        ...
        install.on("close", (code) => {
          if (code === 0) {
            ...
            installSpinner.stop(
              pc.yellow(
                "npm install returned non-zero. You may need to install dependencies manually.",
              ),
            );
        ...
        install.on("error", () => {
          installSpinner.stop(
            pc.yellow("Could not run npm install automatically."),
          );
          resolve();
        });
      });
      ...
      p.outro(
        pc.cyan(
          `🎉 Successfully crafted ${pc.bold(answers.projectName)} using the ${pc.bold(answers.template)} template!`,
        ),
      );
...
program
  ...
  .description(
    "Spin up the generated application locally using Docker Compose.",
  )
